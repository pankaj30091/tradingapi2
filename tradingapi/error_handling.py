...
from .exceptions import (
    TradingAPIError, RetryableError, NonRetryableError, 
    is_retryable_error, create_error_context
)
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    should_retry = (
                        ...
                        (retry_condition is None or retry_condition(e)) and
                        (is_retryable_error(e) if hasattr(e, '__class__') else True)
                    )
...
def log_function_entry_exit(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            trading_logger.log_error(
                f"Exiting {func.__name__} with error",
                e,
                {"function": func.__name__}
            )
            raise
