...
class Config:
    ...
    def __init__(self, default_config_path: str):
        ...
        try:
            ...
            get_trading_logger().log_info("Initializing configuration", {
            ...
            if self.config_path:
                ...
                get_trading_logger().log_info("Configuration initialized successfully", {
                    "config_path": str(self.config_path),
                    "commission_files": len(self.commission_data),
                    "config_keys": list(self.configs.keys())
                })
            else:
                raise ConfigurationError("No valid configuration path found", create_error_context(
                    default_path=str(self.default_config_path),
                    custom_path=str(self.custom_config_path) if self.custom_config_path else None
                ))
    ...
    def _get_custom_config_path(self) -> Optional[Path]:
        ...
        if custom_path:
            ...
            if path.exists():
                ...
                get_trading_logger().log_warning("Custom config path does not exist", {
    ...
    def _validate_config_structure(self) -> None:
        ...
        if missing_keys:
            context = create_error_context(
                ...
                available_keys=list(self.configs.keys())
            )
    ...
    def _validate_commission_data(self) -> None:
        ...
        if not self.commission_data:
            get_trading_logger().log_warning("No commission data loaded", {
                "config_path": str(self.config_path)
            })
            return
        ...
        for effective_date, commission_data in self.commission_data.items():
            if not isinstance(commission_data, dict):
                context = create_error_context(
                    ...
                    data_type=type(commission_data).__name__
                )
    ...
    def load_config(self, config_file: Path) -> None:
        ...
        """
        try:
            get_trading_logger().log_debug("Loading configuration file", {
            ...
            if not isinstance(self.configs, dict):
                context = create_error_context(
                    config_file=str(config_file),
                    config_type=type(self.configs).__name__
                )
                raise ValidationError("Configuration must be a dictionary", context)
                
            get_trading_logger().log_info("Configuration file loaded successfully", {
                "config_file": str(config_file),
                "config_keys": list(self.configs.keys())
            })
    ...
    def load_all_commissions(self) -> None:
        ...
        try:
            ...
            if not commissions:
                get_trading_logger().log_warning("No commission files specified in configuration", {
                    "config_keys": list(self.configs.keys())
                })
                return
            
            get_trading_logger().log_debug("Loading commission files", {
            ...
            get_trading_logger().log_info("All commission files loaded successfully", {
                "commission_count": len(self.commission_data),
                "effective_dates": list(self.commission_data.keys())
            })
            
        except Exception as e:
            context = create_error_context(
                commission_count=len(self.configs.get("commissions", [])),
                error=str(e)
            )
    ...
    def _load_single_commission(self, commission: Dict[str, Any]) -> None:
        ...
        if not isinstance(commission, dict):
            raise ValidationError("Commission entry must be a dictionary", create_error_context(
                commission_type=type(commission).__name__
            ))
        ...
        if not effective_date or not relative_path:
            raise ValidationError("Commission entry must have 'effective_date' and 'file' keys", create_error_context(
                commission=commission
            ))
        ...
        if not commission_file.exists():
            raise FileNotFoundError(f"Commission file '{commission_file}' not found.", create_error_context(
        ...
        with open(commission_file, "r", encoding="utf-8") as file:
            try:
                ...
                if not isinstance(commission_data, dict):
                    raise ValidationError(f"Commission data must be a dictionary", create_error_context(
                        commission_file=str(commission_file),
                        data_type=type(commission_data).__name__
                    ))
                self.commission_data[effective_date] = commission_data
                
                get_trading_logger().log_debug("Commission file loaded", {
                    "effective_date": effective_date,
                    "commission_file": str(commission_file),
                ...
                raise ConfigurationError(f"Error parsing commission YAML file '{commission_file}': {exc}", create_error_context(
                    commission_file=str(commission_file),
                    yaml_error=str(exc)
                ))
    ...
    def get(self, key_path: str, default: Any = None) -> Any:
        ...
        try:
            if not isinstance(key_path, str) or not key_path.strip():
                raise ValidationError("Key path must be a non-empty string", create_error_context(
                    key_path=key_path,
                    key_path_type=type(key_path).__name__
                ))
            ...
            for key in keys:
                if not isinstance(value, dict):
                    get_trading_logger().log_warning("Invalid key path - not a dictionary", {
                        "key_path": key_path,
                        "current_key": key,
                ...
                if key not in value:
                    get_trading_logger().log_debug("Key not found in configuration", {
                        "key_path": key_path,
                        "missing_key": key,
            ...
            get_trading_logger().log_error("Error getting configuration value", e, {
                "key_path": key_path,
                "default": default
            })
            return default
    ...
    def get_commission_by_date(self, target_date: str, key_path: str, default: Any = None) -> Union[float, int]:
        ...
        try:
            ...
            if not target_date or target_date.strip() == "":
                get_trading_logger().log_debug("Empty target date provided", {
            ...
            try:
                ...
                raise ValidationError(f"Invalid date format '{target_date}'. Expected YYYY-MM-DD", create_error_context(
                    target_date=target_date,
            ...
            if not applicable_commission:
                context = create_error_context(
                    target_date=target_date,
                    available_dates=list(self.commission_data.keys())
                )
            ...
            if value is not None and not isinstance(value, (int, float)):
                get_trading_logger().log_warning("Commission value is not numeric", {
                    "target_date": target_date,
                    "key_path": key_path,
                    "value": value,
                    "value_type": type(value).__name__
            ...
            get_trading_logger().log_debug("Commission value retrieved", {
                "target_date": target_date,
                "key_path": key_path,
                "value": value,
            ...
            get_trading_logger().log_error("Error getting commission value", e, {
                "target_date": target_date,
                "key_path": key_path,
                "default": default
            })
    ...
    def _find_applicable_commission(self, target_date: dt.date) -> Optional[Dict[str, Any]]:
        ...
        for effective_date in sorted(self.commission_data.keys(), reverse=True):
            try:
                ...
                get_trading_logger().log_warning("Invalid effective date format", {
                    "effective_date": effective_date,
                    "error": str(e)
                })
                continue
        ...
        if applicable_commission:
            get_trading_logger().log_debug("Found applicable commission", {
    ...
    def _get_value_from_dict(self, data: Dict[str, Any], key_path: str, default: Any = None) -> Any:
        ...
        try:
            for key in keys:
                if not isinstance(value, dict):
                    get_trading_logger().log_debug("Invalid key path - not a dictionary", {
                        "key_path": key_path,
                        "current_key": key,
                        "value_type": type(value).__name__
                    })
                ...
                if key not in value:
                    get_trading_logger().log_debug("Key not found in commission data", {
                        "key_path": key_path,
                        "missing_key": key,
                        "available_keys": list(value.keys())
                    })
            ...
            get_trading_logger().log_error("Error traversing commission data", e, {
                "key_path": key_path,
...
def load_config(default_config_path: str) -> None:
    ...
    try:
        get_trading_logger().log_info("Loading global configuration", {
            "config_path": default_config_path
        })
        
        _config_instance = Config(default_config_path)
        
        get_trading_logger().log_info("Global configuration loaded successfully", {
...
if __name__ == "__main__":
    try:
        ...
        get_trading_logger().log_info("Configuration test completed", {
            "timezone": timezone,
            "data_path": data_path,
        ...
        get_trading_logger().log_info("Commission test completed", {
        ...
        get_trading_logger().log_error("Configuration test failed", e, {
            "script": "config_test"
        })
        sys.exit(1)
